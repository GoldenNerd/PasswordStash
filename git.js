...
function saveCredentialsToLocalStorage(credentialsBeingUpdated) {
  console.log(`I am the function saveCredentialsToLocalStorage().`);
  console.log(`credentials I received from function appendNewlyCreatedCredentialToCredentials() were:`);
  console.log(credentialsBeingUpdated);

  window.localStorage.setItem("credentials", JSON.stringify(credentialsBeingUpdated));
  console.log(`For the time being, I won't reload the page.`);
  //location.reload();
  console.log(`Calling function switchToSearchMode()`);
  switchToSearchMode();
  console.log(`Calling function updatePilotColors()`);
  updatePilotColors();
  alert("Congrats!! \nYour newly created credential was succesfully saved");
};

function appendNewlyCreatedCredentialToCredentials(newlyCreatedCredential) {
  console.log(`I am the function appendNewlyCreatedCredentialToCredentials().`);
  console.log(`The newlyCreatedCredential I got from function makeANewCredential() was:`);
  ...
  saveCredentialsToLocalStorage(credentialsBeingUpdated);
  secretQAsArray2Ready = false;
  return credentialsBeingUpdated;
}
...
function makeANewCredential() {
  ...
  */
  ...
  console.log(`Calling function appendNewlyCreatedCredentialToCredentials()`);
  appendNewlyCreatedCredentialToCredentials(newlyCreatedCredential)
  // saveCredentialsToLocalStorage(credentialsBeingUpdated);
...
function whoRequestedANewCredential() {
  ...
  if (addPilotStyle.backgroundColor == "green") {
    ...
    if (editPilotStyle.backgroundColor == "green") {
      ...
      if (confirm('⚠  Warning!\nYour newly edited credential will replace your current versionl\nProceed?')) {
        alert('Your outdated credential will be replaced with your newly edited versionl');
...
function modifyACredential() {
  ...
  console.log(`Calling function queryACredential()`);
  queryACredential(siteNameToQuery);
  let indexOfMatchedCredential = JSON.parse(window.sessionStorage.getItem("indexOfMatchedCredential"));
  console.log(`Calling function retrieveCredentialsFromLocalStorage()`);
  retrieveCredentialsFromLocalStorage(); // Reload the credentialsRetrieved localstorage mirror variable
  let credentialsRetrievedClone = credentialsRetrieved.slice();
  // delete old credential
  console.log(`Deleting website Name ${siteNameToQuery}`);
  credentialsRetrievedClone.splice(indexOfMatchedCredential, 1);
  var credentialsBeingUpdated = credentialsRetrievedClone.slice();
  console.log(`Calling function saveCredentialsToLocalStorage()`);
  saveCredentialsToLocalStorage(credentialsBeingUpdated);
  console.log(`Calling function retrieveCredentialsFromLocalStorage()`);
  retrieveCredentialsFromLocalStorage(); // Reload the credentialsRetrieved localstorage mirror variable
  ...
  var credentialsBeingUpdated = appendNewlyCreatedCredentialToCredentials(newlyCreatedCredential);
  saveCredentialsToLocalStorage(credentialsBeingUpdated)
}
// *  #########
...
function queryACredential(siteNameToQuery) {
  ...
  if (credentialsArrayContainsData) {
    ...
    for (let index = 0; index < credentialsRetrieved.length; index++) {
      ...
      if (aCredential.name == siteNameToQuery) {
        ...
        window.sessionStorage.setItem("indexOfMatchedCredential", JSON.stringify(index));
      }
    }
    if (userSelectedSiteNameFound) {
      window.sessionStorage.setItem(
        "credentialNeeded",
        JSON.stringify(aCredential));
    } else {
      alert(
...
function spoolToDisplay() {
  ...
  let credentialNeeded = queryACredential(siteNameToQuery);
...
// Delete a credential
function deleteCredential() { // OK - It is deleting fine
  retrieveCredentialsFromLocalStorage(); // Reload the credentialsRetrieved localstorage mirror variable 
