...
function saveCredentialsToLocalStorage(credentials) {
  ...
  console.log(
    `credentials I received from function   appendNewlyCreatedCredentialToCredentialsInLS() were:`
  );
  console.log(credentials);

  window.localStorage.setItem("credentials", JSON.stringify(credentials));
  console.log(
    `I am the function saveCredentialsToLocalStorage(). For the time being, I won't reload the page.`
  );
  //location.reload();
...
function appendNewlyCreatedCredentialToCredentialsInLS(newlyCreatedCredential) {
  console.log(
    `I am the function  appendNewlyCreatedCredentialToCredentialsInLS().`
  );
  console.log(
  ...
  //Improove: the following line should not be neede
...
function makeANewCredential() {
  ...
  */
  ...
  console.log(`newlyCreatedCredential:`);
  console.log(newlyCreatedCredential);
  // console.log(`Calling function  appendNewlyCreatedCredentialToCredentialsInLS()`);
  // saveCredentialsToLocalStorage(credentials);
...
function whoRequestedANewCredential() {
  ...
  if (addPilotStyle.backgroundColor == "green") {
    ...
    appendNewlyCreatedCredentialToCredentialsInLS(newlyCreatedCredential);
  } else {
...
function modifyACredential() {
  ...
  if (indexOfMatchedCredential == credentialsRetrieved.length + 1) {
    ...
    function stashOnScreenDataAsNewCredential() {
      ...
      console.log(
        `Calling function  appendNewlyCreatedCredentialToCredentialsInLS()`
      );
      appendNewlyCreatedCredentialToCredentialsInLS(newlyCreatedCredential);
      /*
...
function clrSearchBar() {
  // search bar should not get focus by clicking outside if credentials array is empty
...
//appBody.addEventListener("click", clrSearchBar);
...
function gateToSearchFunctionality() {
  if (credentialsRetrieved == null || credentialsRetrieved.length == 0) {
    switchToCreateModeScreen();
  } else {
...
const searchBar = document.getElementById("search-bar");
searchBar.addEventListener("click", gateToSearchFunctionality);
searchBar.addEventListener("keyup", gateToSearchFunctionality);
searchBar.addEventListener("focus", gateToSearchFunctionality); // ??
