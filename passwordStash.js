...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%OK
// If there were no questions or answers to show, the Q-A section heading showld not display.
function showOrHideQAHeading() {
  const siteNameSelectedForViewing = document.getElementById("name").value;
  const credentialNowOnDisplay = queryACredentialBySiteName(
    siteNameSelectedForViewing
  );
  const arrayWithQsAndAs = credentialNowOnDisplay.secretQAs;
  let allQuestionsArray = [];
  let allAnswersArray = [];
  for (
    let pointerToAQuestionAndItsAnswerObj = 0;
    pointerToAQuestionAndItsAnswerObj < arrayWithQsAndAs.length;
    pointerToAQuestionAndItsAnswerObj++
  ) {
    const oneOfTheQuestions =
      arrayWithQsAndAs[pointerToAQuestionAndItsAnswerObj].secretQ;
    allQuestionsArray.push(oneOfTheQuestions);
    const oneOfTheAnswers =
      arrayWithQsAndAs[pointerToAQuestionAndItsAnswerObj].secretA;
    allAnswersArray.push(oneOfTheAnswers);
  }

  const allQuestionsAndAnswersArray = allQuestionsArray.concat(allAnswersArray);
  console.log(`allQuestionsAndAnswersArray:`);
  console.log(allQuestionsAndAnswersArray);
  // let showQAHeading;
  for (let index = 0; index < allQuestionsAndAnswersArray.length; index++) {
    let element = allQuestionsAndAnswersArray[index];
...
function spoolToDisplay(siteNameSelectedForViewing) {
  ...
  //############## values for create/edit mode: #############################
  //For the credentialToShowOnDisplay, inject the credential's values into their corresponding text input fields of the DOM
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%OK
function hideAllTextInputFields() {
  console.log(`I am the function hideAllTextInputFields()`);
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%OK
function hideAllInnerHTMLValues() {
  console.log(`I am the function hideAllInnerHTMLValues()`);
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%OK
function hideQAHeading() {
  console.log(`I am the function hideQAHeading()`);
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%OK
function showQAHeading() {
  console.log(`I am the function showQAHeading()`);
...
function switchToEditModeScreen() {
  ...
  if (
    ...
    // Not green = not in edit mode. Therefore, switch to Edit Mode Screen.

    // "Illuminate/focus on" Edit Button
    ...
    // Show Edit Mode anouncement
    searchModeAnouncementStyle.display = "none";
    viewModeAnouncementStyle.display = "none";
    editModeAnouncementStyle.display = "block";
    createModeAnouncementStyle.display = "none";
    // Update Developer Dashboard onscreen mode indicators
    updateAppModeIndicatorColors();

    // Display abd configure Edit Mode Toolbar buttons
    ...
    showAllInputFields();
    showQAHeading();
    // Locate cursor on website name input field
    ...
    // Is green = already in Edit Mode Screen. Do nothing, and remain in Edit Mode Screen
  }
}
...
function turnOffEditButton() {
  ...
  // Unfocus/turn off Edit Button Appearance
  const pencilIconStyle = document.getElementById("pencil-icon").style;
  const pencilBtnStyle = document.getElementById("editCredential").style;
  pencilIconStyle.color = "#00243f";
...
// Hide empty innerHTML values on Display Mode Screen
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%CHK
// Improove: Use high order function
function doNotDisplayEmptyValues() {
  ...
  for (
    ...
    // Spans with no text won't show on screen. Spans with content will show on screen.
    if (spanTextContent == "") {
...
add.addEventListener("click", switchToCreateModeScreen);
