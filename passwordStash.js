...
function testAFunction() {
  ...
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  function dummyConcealer8() {
    /*
let searchBarContentAfterClick;
  function wasSearchBarXClicked() {
    const searchBarContentBeforeClick = document.getElementById("search-bar")
      .value;
    searchBarContentAfterClick = readSearchContentAfterClick();
    let SearchBar_xWasClicked;
    if (searchBarContentBeforeClick !== searchBarContentAfterClick) {
      SearchBar_xWasClicked = true;
      console.log(
        `The search bar was cleared by a click.\nThis means the search bar "x" (little clr button) was clicked.\nProceed to clear the 3 search results.`
      );
    } else {
      SearchBar_xWasClicked = false;
      //Do nothing. There is already an event listener that detects this condition, and handles it.
    }
    console.log(`\nsearchBarContentBeforeClick:`);
    console.log(searchBarContentBeforeClick);
    console.log(`\nsearchBarContentAfterClick:`);
    console.log(searchBarContentAfterClick);
    console.log(`\nSearchBar_xWasClicked: ${SearchBar_xWasClicked}`);

    return SearchBar_xWasClicked;
  }
  wasSearchBarXClicked();
};
*/
  }
  function dummyConcealer7() {
    /*
  const searchBarTester = document.getElementById("search-bar");
searchBarTester.addEventListener("click", function () {
  //let searchBarContentAfterClick;
  setTimeout(function () {
    searchBarContentAfterClick = document.getElementById("search-bar").value;
  }, 1000);
  console.log(`\nsearchBarContentAfterClick:`);
  console.log(searchBarContentAfterClick);
  return searchBarContentAfterClick;
});
// function readSearchContentAfterClick() {}
*/
  }
}
...
function copyToClipboard(e) {
  ...
  document.getElementById(
    "notifications"
  ).innerHTML = `"${textOnViewModeScreen}" Copied to clipboard`;
...
function saveCredentialsToLocalStorage(credentials) {
  ...
  console.log(
    `credentials I received from function   append_newlyCreatedCredentialTo_credentialsRetrieved() were:`
  );
  ...
  console.log(`I will NOT initialize the App.`); // For the time being, I won't reload the page.
  initializeApp();
  //location.reload();
...
function test1BeforeMakingNewCredential() {
  console.log(`Step #3 test1BeforeMakingNewCredential()();`);
