...
function retrieveFromLocalStorage() {
  const credentialsRetrieved = JSON.parse(
...
const credentialsRetrieved = retrieveFromLocalStorage();
  ...
  if (addPilotStyle.backgroundColor == "green") {
    ...
    */
    ...
    if (editPilotStyle.backgroundColor == "green") {
      // Replace credential with edited credential
      console.log(`Replace credential with edited credential`);
    }
  }
...
function queryACredential() {
  ...
  if (thereIsDataInCredentialsArray) {
    ...
    window.sessionStorage.setItem(
      "credentialsArrayIsInLocalstorage",
      JSON.stringify(credentialsArrayIsInLocalstorage)
    );
    window.sessionStorage.setItem(
      "credentialsArrayIsInLocalstorage",
      JSON.stringify(credentialsArrayIsInLocalstorage)*/
...
console.log(`2) credentialsArrayIsPregnant();`);
  let credentialsArrayIsPresentInLocalstorage = credentialsArrayExistsInLocalstorage();

  function Does_credentialsArrayExistsInLocalstorage() {
  retrieveFromLocalStorage();
  console.log(`1) Running "does_credentialsArrayExistsInLocalstorage()" function.;
  arrayInLocalStorage: ${credentialsRetrieved}`);
...
function is_credentialsArrayEmpty() {
  let credentialsArrayIsEmpty;
  console.log(`2) Running "is_credentialsArrayEmpty" function.;
  arrayInLocalStorage: ${credentialsRetrieved}`);
  let credentialsArrayIsPresentInLocalstorage = does_credentialsArrayExistsInLocalstorage();
  if (credentialsArrayIsPresentInLocalstorage == false) {

    alert(`Be adviced that credentialsArray is missing from localstorage.
    localStorage contents: ${credentialsRetrieved}
    As your 1st step, you must create and save at least one password.
    I will now switch to the "Create a Aew Password Mode".`);
    ...
    if (credentialsRetrieved.length == 0) {
      ...
      alert(`Be adviced that credentialsArray is present in localstorage
    (localStorage contents: ${credentialsRetrieved}).
  ...
  return credentialsArrayIsEmpty;
}
*/
// ###############################################
...
let searchModeAnouncementStyle = document.getElementById("search-mode-anouncement")
  .style;
...
let editModeAnouncementStyle = document.getElementById("edit-mode-anouncement")
  .style;
let createModeAnouncementStyle = document.getElementById("create-mode-anouncement")
  .style;
  ...
  if (nameOfSiteToBeDeleted == "") {
    ...
    for (let index = 0; index < credentialsRetrieved.length; index++) {
      ...
      if (nameOfSiteToBeDeleted == aCredential.name) {
        ...
        credentialsRetrieved.splice(index, 1);
        let arrayAfterTrim = credentialsRetrieved.slice();
