...
function appWorkflow() { // APP WORKFLOW DESCRIPTION:
  console.log(`/* 
...
function makeNewCredentialAndSaveIt() {
  ...
  const newWebsiteUsualValues = [name, website, email, username, password, pin, recoveryEmail, secretImage];
  // console.log(`newWebsiteUsualValues:`);
  // console.log(newWebsiteUsualValues);
  // STEP 1 - b) User "SAVE CREDENTIALS" button results in the following "Question-Answer" values:
  const secretQ = document.querySelectorAll('.question')[0].value;
  const secretA = document.querySelectorAll('.answer')[0].value;
  ...
  } = new WebsiteUsualProperties(newWebsiteUsualValues);
...
function addXtraQA() {
  const secretXtraQ = document.querySelectorAll('.question')[0].value;
  const secretXtraA = document.querySelectorAll('.answer')[0].value;
  ...
  } = new ASecretQA(newWebsiteXtraQAValues);
...
function spoolToDisplay() {
  ...
  document.getElementById('recovery-email').value = credentialNeeded.recoveryEmail;
  document.getElementById('secret-image').value = credentialNeeded.secretImage;

  document.getElementsByClassName('question')[0].value = credentialNeeded.secretQAs[0].secretQ;
  document.getElementsByClassName('answer')[0].value = credentialNeeded.secretQAs[0].secretA;
}
